...
class BazarperfilesGenerator < Rails::Generators::Base
...
# vemos si ya existe un fichero de migracion v1
    ...
    Dir.foreach("db/migrate") { |f|
      if File.fnmatch('*_create_bazarperfiles_tables.rb', f) then
        puts 'existe!!!!'
        existe = true
        end
      }
    ...
    if (existe == false) then
      migration_template  '../../../tmp/~migration_ready.rb','db/migrate/create_perfiles_tables.rb'
    end
...
#      existe = false; 
#      Dir.foreach("db/migrate") { |f|
#        if File.fnmatch('*_create_bazarperfiles_tables'+i.to_s+'.rb', f) then
#          puts 'existe!!!!'
#          existe = true
#        end
#      }
...
#        if (existe == false) then
#          migration_template  File.join(File.dirname(__FILE__), 'templates', 'schema'+i.to_s+'.rb'),'db/migrate/create_bazarperfiles_tables'+i.to_s+'.rb'
#        end
#    end
  ...
  def copy_initializer_file
    copy_file 'initializer.rb', 'config/initializers/bazarperfiles.rb'
  end
...
layout = '  <% content_for :bazarcontent do %>
    <%= stylesheet_link_tag("bazarperfiles") %>
    <div> 
      <%= yield %>
    </div>
  <% end -%>
...
copy_file '../../../tmp/~application.html.erb', 'app/views/layouts/bazarperfiles.html.erb'
